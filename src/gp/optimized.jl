...
export create_optim_gp, create_optim_gpar, create_optim_gp_post, create_optim_gpar_post
...
function create_optim_gp(
    ...
    opt_params = unpack_gp(results.minimizer)
    opt_l, opt_process_var, opt_noise_sigma = opt_params
    if debug
    ...
    return gp, opt_params
end


"""
Compute the optimised posterior for a GP.
...
function create_optim_gp_post(
    ...
    gp, opt_params = create_optim_gp(
        input_locations,
        outputs;
        kernel_structure=kernel_structure,
        # Initial log vals for hyperparameters which will be optimized
        i_log_l=i_log_l, i_log_process_var=i_log_process_var,
        i_log_noise_sigma=i_log_noise_sigma,
        debug=debug,
    )
    opt_noise_sigma = opt_params[3]
...
"""
Compute the optimised GPAR.

input_locations: array of length L
outputs: array of length L
"""
function create_optim_gpar(
    input_locations,
    outputs;
    ...
    return gpar, opt_params
end


"""
Compute the optimised posterior for a GPAR.
...
function create_optim_gpar_post(
    ...
    gpar, opt_params = create_optim_gpar(
        input_locations,
        outputs;
        ...
        multi_input=multi_input,  # False if it is the first gpar kernel
        debug=debug,
    )
    input_locations = to_ColVecs(input_locations)  # transform into ColVecs
    opt_noise_sigma = opt_params[5]
